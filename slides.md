...
Today I would like to share with you the experience we had in dealing with microservices. I believe that we created a very nice system and that we learned a lot by making it. I am really happy and excited to be able to share it with you and hope that you will enjoy this talk as much as I will.

# 2 

My name is Marin Prcela. I am a senior lead developer at minus5 here in Zagreb. Our team has 20 people. Most of us are developers and designers. So we are very much technically oriented. We are working on sports and betting techologies. This is our team (picture).

# 3

Our most significat client (and at the same time our partner) is SuperSport betting company also from Croatia. SuperSport is the largest betting company in Croatia (20TB monthly data transfer, 9M monthly business transactions). It started 12 years ago with several bet-shops. It was the first company in Croatia to introduce betting machines in public places (10 years ago) and also the first one to introduce online betting on the first day it became legally possible (7 years ago). Today SuperSport holds the dominant position in the betting industry in Croatia.
...
Since I am a developer I had the need to to leave you with someting you can easily try yourself on your local machine. So I have created a couple of examples that go along with this talk. You can find them here. I have tested them on mac and on ubuntu and everything worked as expected so feel free to try the out. But please don't do it during this talk, I don't want to lose your attention.
...
If you do take a look at the examples (later) you will find that they are written in Go. Go is the language we use for, well more than 90% of services in our backend. We have been using is since the very beginnings, like X years ago, and we have found it to be very appripriate for backend tasks. Also, somehow plenty of other tools that we use have also been written using Go. Of course it always depends on the context but for us at this moment Go is the "way to go".

# 6

Just to make sure that we are all on the same page I would like to give a quick introduction to microservices. The term microservices is not new and everyone seems to now what it means by now, but for me it suprisingly turned out to be a bit different than I thought it meant the first time I started to dig a bit more deeply into the matter.
...
- that thay **implement business capabilities**, which means NOT organizing stuff around technical features but around business feratures,
...
So each of those four crucial points is not about techology but about somethig completely on the other side of the spectrum, like autonomy, business, continuos delivery, evolving tech stack and agility. 

So in case you thought microservices are about some technical aspects you were wrong, they are much more abstract term which talks more about organizing your system and your teams than the techology itself.
...
I run into a beautiful question from Greg Young's talk which goes like this:
...
If you listened carefully you might have noticed that now it is not possible for one service to reach into another services database. So what happens it your manager comes in ione day and asks you to make a quick fix by grabbing that data from another database?
...
Hopefuly that shuold be enough to give you the idea what are we trying to solve and get us on the same page. It's time to move on to the examples I promised.
...
Event though messaging is the preferred way to communicate we still have a lot of communication done using REST interfaces. Also, there are some components in the system that are not able to communicate using messaging. For example there are databeses, key-value storages, web proxies, externale web services and so on. 
...
And this is how it looks like. Here is another This is our alerts room in Slack top which we send all notifications from consul.
...
Now, I wouldn't like to go into details of how Docker works. The reason whuy I am telling you this is that by using this we have all of our infrastructure written in source code. 
